...
**真实代码审查 (/10review)**。旧版审查只是 10 规则合规清单。现在加入了自检门：如果 agent 给了 PASS 但没有引用具体的 `file:line` 证据，自动触发深度代码审查（逻辑、安全、边界条件、错误处理）。P1 级发现直接阻塞发布。

**环境感知执行 (/10exec)**。启动时自动检测测试框架（jest/vitest/pytest/go/cargo/shell）、linter 和类型检查器。每个阶段结束后跑测试，输出具体的 pass/fail 数字。不再是空洞的"测试通过"。

**阶段级代码审查**。/10exec 每个阶段完成后自动跑精简版审查（R5 隔离 + R7 失败路径 + R9 验证 + 深度代码审查）。P1 级发现自动阻塞并触发自我纠正。全量 /10review 在所有阶段完成后运行。

**共享工具**。`bin/detect-root.sh`（项目根检测 + glob 兜底）和 `bin/detect-env.sh`（环境检测）提取为共享脚本，7 个 skill 不再重复。

**边界守卫增强**。`realpath` 规范化防止 `../` 路径遍历。`.10dev/` 状态文件豁免边界检查。
...
v2.4 采用**路由层架构** + 三层学习系统 + 真实代码审查 + 环境感知执行。
